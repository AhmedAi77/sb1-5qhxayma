        ...
        >
          ...
          <div className="flex justify-between items-center relative z-10">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-display text-cyan-400 drop-shadow-sm">{t('stats.title')}</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="stat-card col-span-2 relative">
              <div className="stat-value text-cyan-400">{stats.totalExercises}</div>
              <div className="stat-label text-purple-200">{t('stats.totalExercises')}</div>
            ...
            <div className="stat-card relative">
              <div className="stat-value text-cyan-400">{stats.totalReps}</div>
              <div className="stat-label text-purple-200">{t('stats.totalReps')}</div>
              <div className="absolute top-3 right-3 text-purple-400/70">
            ...
            <div className="stat-card relative">
              <div className="stat-value text-cyan-400">{stats.totalDistance}m</div>
              <div className="stat-label text-purple-200">{t('stats.totalDistance')}</div>
              <div className="absolute top-3 right-3 text-purple-400/70">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-display text-cyan-400">{t('stats.personalBests')}</h3>
                ...
                <div className="space-y-3">
                  {Object.entries(stats.personalBests)
                    ...
                    .map(([exercise, reps], index) => (
                      ...
                      >
                        <div>
                          <div className="text-sm font-medium text-cyan-400">{exercise}</div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-lg font-bold text-cyan-400">{reps}</span>
                        </div>
                      </motion.div>
                    ))}
                </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-display text-cyan-400">{t('stats.recentActivity')}</h3>
              <div className="flex items-center gap-1 bg-purple-600/20 px-2 py-1 rounded-lg">
                <Clock className="w-3.5 h-3.5 text-purple-400" />
                <span className="text-xs text-purple-400">{stats.exerciseHistory.length}</span>
              </div>
            </div>
                    ...
                    .map((activity, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-sm font-medium text-cyan-400">{activity.exercise}</div>
